...
foreign_t swi_pgsql_connect2(term_t p_hostname, term_t p_port, term_t p_user, term_t p_passwd, term_t p_dbname, term_t dbx) 
{
	...
	PL_get_chars(p_hostname, &hostname, CVT_ATOM | CVT_STRING | CVT_LIST | BUF_RING);
	PL_get_chars(p_port, &port, CVT_ALL | BUF_RING);
	PL_get_chars(p_dbname, &dbname, CVT_ATOM | CVT_STRING | CVT_LIST | BUF_RING);
	if (PL_term_type(p_user) != PL_VARIABLE)
	{
		PL_get_chars(p_user, &user, CVT_ATOM | CVT_STRING | CVT_LIST | BUF_RING);
	}
	if (PL_term_type(p_passwd) != PL_VARIABLE)
	{
		PL_get_chars(p_passwd, &passwd, CVT_ATOM | CVT_STRING | CVT_LIST | BUF_RING);
	}
	PL_get_chars(p_port, &port, CVT_ALL | BUF_RING);
...
foreign_t swi_pgsql_query1 (term_t dbx, term_t query)
{
  ...
  unsigned long t_val;
	...
	PL_get_chars(query, &qstr, CVT_ATOM | CVT_STRING | CVT_LIST | BUF_RING);
...
foreign_t swi_pgsql_query2 (term_t dbx, term_t query, term_t my_res, foreign_t handle) 
{
	...
	term_t val =  PL_new_term_ref();
	term_t lst = PL_copy_term_ref(my_res);

	/* copy single row to list */
        int fnum = 0;
        while (fnum < result->nfields) {
		char *tchar;
		tchar = PQgetvalue(result->result,result->atrow,fnum);
		PL_unify_list(lst, val, lst);
               /* printf ("got: type %d  value %s\n", PQftype((*result),fnum), tchar);  */
               switch (PQftype(result->result, fnum)) {
                        case INT2OID:
                        case INT4OID:
                        case OIDOID:
				//printf("                             integer\n");
	...
	result->atrow++;
	int retval = PL_unify_nil(lst);

	if (result->atrow >= result->nrows)
	{
		PQclear(result->result);
		free(result);
		return retval;		// stop here
	}
